...
public class ArcherTest {
    ...
    @Test
    void checkTotalMisses() {
        int[] oneMissPerRound = new int[]{9,9,0};
        int[] twoMissesPerRound = new int[]{0,10,0};
        int[] noMisses = new int[]{4,4,4};
        for(int round = 1; round <= Archer.MAX_ROUNDS; round++) {
            archer1.registerScoreForRound(round, oneMissPerRound);
            archer2.registerScoreForRound(round, twoMissesPerRound);
            archer3.registerScoreForRound(round, noMisses);
        }
        assertSame(1 * Archer.MAX_ROUNDS, archer1.getTotalMisses());
        assertSame(2 * Archer.MAX_ROUNDS, archer2.getTotalMisses());
        assertSame(0, archer3.getTotalMisses());
    }
}
